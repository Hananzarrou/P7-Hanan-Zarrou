<template>
    <div class="footer bg-light font-weight-bold text-secondary">
        <footer>
            
            <ul class="list-inline">
                <li class="list-inline-item"><a class="font-weight-bold text-secondary" href="#">Mentions légales</a></li>
                <li class="list-inline-item"><a class="font-weight-bold text-secondary" href="#">Signaler un bug</a></li>
                <li class="list-inline-item"><a class="font-weight-bold text-secondary" href="/myprofile">voir mon  profile</a></li>
            </ul>
            <p class="copyright">Groupomania © 2021</p>
        </footer>
    </div>
</template>


<script>
export default {
    name: 'Footer',
}
</script>

<style scoped>
 .Footer {
    margin-top: 20px;
    
 }
</style>